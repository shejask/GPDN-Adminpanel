...
export function CreateThreadDialog({ open = false, onOpenChange }: CreateThreadDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      submitFormData.append('authorId', formData.authorId || "661fb221a2c4a7d43f2f12ab")
      ...
      const response = await fetch("https://api.thegpdn.org/api/thread/AddThread", {
        method: "POST",
      ...
      setIsOpen(false)
      setFormData({ title: "", content: "", authorId: "", thumbnail: "" })
      setTags([])
      setTagInput("")
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new thread in the forum. Fill in the details below.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter className="mt-4">
            ...
            <Button 
              type="submit" 
              disabled={loading || !formData.title || !formData.content || !formData.authorId || tags.length === 0}
              className="gap-2"
            >
              {loading ? (
                <>
